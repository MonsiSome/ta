...
function calcTextEditCost(text, lang, type = '.docx') {
  let costPerSymbols,
    minCost,
    totalCost;

  // get cost per symbol depending on language
  minCost = lang === 'EN' ? MIN_EN_COST : MIN_RU_UKR_COST
  ...
  if (basicTypesOfFile.some(basicType => basicType === type)) {
    costPerSymbols = languages[lang]
  } else {
    costPerSymbols = Number((languages[lang] + languages[lang] * PERCENT_FOR_EXTRA_RESOURCES / 100).toFixed(4))
  }

  // check if the task can be done on minimal price or calculate total price
  if (text.length * costPerSymbols < minCost) {
    totalCost = minCost
  } else {
    totalCost = text.length * costPerSymbols
  }

  return Math.ceil(totalCost) // нужно ли округлять копейки к гривням? я округлила
}

console.log(calcTextEditCost('text1400', 'EN', '.docx'))

// calculate the time of text editing in minutes
function calcTextEditHours(text, lang, type = '.docx') {
  let minPerSymbols,
      extraTime = 0,
      totalTimeInMin;
  ...
  if (lang === 'EN') {
    minPerSymbols = text.length / SYMBOLS_EN_PER_HOUR * 60
  } else {
    minPerSymbols = text.length / SYMBOLS_RU_UKR_PER_HOUR * 60;
  }

  // check if we need to add 20% to business time because of file format
  if (!basicTypesOfFile.some((basicType) => basicType === type)) {
    extraTime = minPerSymbols * PERCENT_FOR_EXTRA_RESOURCES / 100
  }

  // check if the task can be done on minimal time or calculate total business time
  if (OPEN_ORDER_PROCESSING_TIME_IN_MIN + extraTime + minPerSymbols < MIN_MINUTES_PER_ORDER) {
    totalTimeInMin = MIN_MINUTES_PER_ORDER;
  } else {
    totalTimeInMin = Math.ceil(OPEN_ORDER_PROCESSING_TIME_IN_MIN + extraTime + minPerSymbols)
  }

  return totalTimeInMin;
}

console.log(calcTextEditHours('text1400', 'EN', '.docx'));
...
console.log(getDeadlineForOrder(calcTextEditHours('text1400', 'EN', '.docx')));
